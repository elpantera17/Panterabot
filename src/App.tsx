...
const App: React.FC = () => {
  ...
  const [botService] = useState(() => AndroidNativeService.getInstance());
  ...
  const [permissions, setPermissions] = useState<Permission[]>([
    {
      ...
      description: 'Necesario para detectar viajes cercanos automáticamente',
      granted: false,
      action: 'location',
    ...
    },
    {
      ...
      description: 'Permite al bot interactuar automáticamente con inDrive',
      granted: false,
      action: 'accessibility',
      instructions: '1. Ve a Configuración → Accesibilidad\n2. Busca "Pantera Bot Automático" en Apps descargadas\n3. Activa el servicio de accesibilidad\n4. Esto permite la automatización completa'
    },
    {
      name: 'Notificaciones',
      icon: <Bell className="w-5 h-5" />,
      description: 'Para alertas automáticas de viajes y estado del bot',
      granted: false,
    ...
    },
    {
      ...
      description: 'Permite mostrar el bot automáticamente sobre inDrive',
      granted: false,
      action: 'overlay',
      instructions: '1. Ve a Configuración → Apps especiales\n2. Selecciona "Mostrar sobre otras apps"\n3. Busca "Pantera Bot Automático" y actívalo\n4. Esto permite funcionamiento automático'
    },
    {
      name: 'Ejecutar en segundo plano',
      icon: <Shield className="w-5 h-5" />,
      description: 'Mantiene el bot funcionando automáticamente 24/7',
      granted: false,
      action: 'background',
      instructions: '1. Ve a Configuración → Batería\n2. Selecciona "Optimización de batería"\n3. Busca "Pantera Bot Automático" y selecciona "No optimizar"\n4. Esto permite funcionamiento automático continuo'
    }
  ]);
  ...
  useEffect(() => {
    ...
    // Inicializar servicio automático
    if (Capacitor.isNativePlatform()) {
      initializeAutomaticService();
    }
  }, []);

  const initializeAutomaticService = async () => {
    try {
      await botService.initializeBot(config);
      console.log('✅ Servicio automático inicializado');
    } catch (error) {
  ...
  const saveConfig = async () => {
    ...
    if (isNativeApp) {
      // Actualizar configuración en servicio nativo
      await botService.updateConfig(config);
      await AndroidPermissions.showNotification(
        'Configuración Automática Guardada', 
        'La configuración se ha guardado y aplicado automáticamente'
      );
    } else {
  ...
  const toggleBot = async () => {
    ...
    if (!allPermissionsGranted) {
      const message = '⚠️ Necesitas configurar todos los permisos antes de iniciar el bot automático';
      if (isNativeApp) {
    ...
    if (!botActive) {
      ...
      const success = await botService.startBot();
      if (success) {
        setBotActive(true);
        const message = config.autobidEnabled 
          ? '🤖 Pantera Bot Automático iniciado - Detectando y ofertando viajes automáticamente'
          : '🤖 Pantera Bot iniciado - Solo funciones de filtrado automático activas';
        
        if (isNativeApp) {
          await AndroidPermissions.showNotification('Bot Automático Iniciado', message);
        } else {
          alert(message);
        }
      }
      ...
      const message = '⏹️ Pantera Bot Automático detenido';
      if (isNativeApp) {
        await AndroidPermissions.showNotification('Bot Detenido', message);
      } else {
        alert(message);
      }
    }
  ...
  const AnimatedTitle = () => {
    ...
    return (
      ...
      >
        🤖 Pantera Bot AUTOMÁTICO {isNativeApp && <span className="text-green-400 text-sm">📱 Android</span>}
      </motion.h1>
    );
  };
  ...
  const ContactAdmin = () => {
    ...
    return (
      <motion.button
        ...
        onClick={() => {
          if (isNativeApp) {
            AndroidPermissions.openUrl('https://wa.me/18098522664');
          } else {
      ...
      <div className="max-w-md mx-auto">
            ...
            <div className="flex items-center space-x-2">
              {permissions.every(p => p.granted) ? (
                <>
                  ...
                  <span className="text-green-300 font-semibold">✅ Todos los permisos configurados - Bot 100% automático</span>
                </>
              ) : (
                <>
                  <X className="w-5 h-5 text-red-400" />
                  <span className="text-red-300 font-semibold">
                    ⚠️ {permissions.filter(p => !p.granted).length} permisos pendientes para automatización
                  </span>
                </>
              )}
            </div>
        ...
        <div className="mb-8">
          <div className="bg-gradient-to-r from-orange-800 to-red-800 rounded-lg p-4 border-2 border-orange-500 shadow-lg">
            <FilterCheckbox
              label="🤖 Autobid List - AUTOMATIZACIÓN COMPLETA"
            ...
            <p className="text-xs text-orange-200 mt-2 ml-8">
              {config.autobidEnabled 
                ? "✅ El bot detectará y ofertará automáticamente SIN intervención manual" 
                : "❌ Solo funciones de filtrado automático (sin ofertar)"}
            </p>
          </div>
        </div>
        ...
        <div className="space-y-4 mb-8">
          <FilterCheckbox
            label="🚫 Hide - Ocultar automáticamente viajes con múltiples destinos"
          ...
          <FilterCheckbox
            label="❌ Rechazar automáticamente clientes nuevos"
          ...
          <div className="bg-gray-800 rounded-lg p-4 border border-orange-500">
            <FilterCheckbox
              label="⭐ Filtrar automáticamente por calificación mínima del pasajero"
            ...
            {config.filters.rejectLowRating && (
              ...
              >
                <div className="flex items-center space-x-3">
                  ...
                  <span className="text-white text-sm">Calificación mínima automática:</span>
        ...
        {/* Configuración de precios automáticos - Solo visible si Autobid está activado */}
        {config.autobidEnabled && (
          <div className="space-y-4 mb-8">
            <div className="bg-orange-900 bg-opacity-30 rounded-lg p-4 border border-orange-500">
              <h3 className="text-orange-300 font-semibold mb-4 flex items-center space-x-2">
                ...
                <span>💰 Configuración de Ofertas Automáticas</span>
              </h3>
              
              <InputField
                label="Precio automático por KM"
              ...
              <div className="grid grid-cols-2 gap-4 mt-4">
                <InputField
                  label="Precio Mínimo Automático"
                  value={config.minPrice}
                  onChange={(value) => setConfig(prev => ({ ...prev, minPrice: value }))}
                  icon="⬇️"
                />
                <InputField
                  label="Precio Máximo Automático"
        ...
        <div className="mb-8">
          <div className="bg-gray-800 rounded-lg p-4 border border-cyan-500">
            <div className="flex items-center space-x-3">
              ...
              <span className="text-white text-sm">🔄 Refresh - Velocidad de análisis automático:</span>
              <div className="flex-1">
                <input
                  type="range"
                  min="300"
            ...
            <p className="text-xs text-gray-400 mt-2">Analiza automáticamente la pantalla cada {config.autoRefresh}ms</p>
          </div>
        </div>
        ...
        <div className="space-y-4">
          <motion.button
            ...
            whileTap={{ scale: 0.98 }}
          >
            {permissions.every(p => p.granted) ? '✅ PERMISOS CONFIGURADOS - BOT 100% AUTOMÁTICO' : '🔐 CONFIGURAR PERMISOS PARA AUTOMATIZACIÓN'}
          </motion.button>
          ...
          >
            ...
            <span>{botActive ? '⏹️ DETENER BOT AUTOMÁTICO' : '🤖 INICIAR BOT AUTOMÁTICO'}</span>
          </motion.button>
        </div>
        ...
        <div className="mt-8 text-center">
          <p className="text-gray-400 text-sm">
            🤖 Pantera Bot v2.0 AUTOMÁTICO {isNativeApp ? '📱 Android Native' : '🌐 Web Version'}
          </p>
          <p className="text-orange-400 text-xs mt-1">
            ✨ Detección y ofertas 100% automáticas sin intervención manual
          </p>
        </div>
      </div>
    </div>
    ...
    >
      ...
      >
        <h2 className="text-xl font-bold text-white mb-6 text-center">
          🔐 Configurar Permisos para Automatización {isNativeApp && <span className="text-green-400 text-sm">📱</span>}
        </h2>
        ...
        {selectedPermission && (
          <>
            ...
            <div className="bg-gray-800 rounded-lg p-4 mb-6">
              <h3 className="text-white font-semibold mb-3">📋 Instrucciones para Automatización:</h3>
            ...
            <div className="space-y-3">
              ...
              >
                ✅ Ya configuré este permiso para automatización
              </button>
